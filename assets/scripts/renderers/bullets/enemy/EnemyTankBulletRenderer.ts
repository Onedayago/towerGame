...
/**
 * 坦克子弹渲染器
 * 负责坦克子弹的绘制逻辑（美化版：多层发光效果）
 */
export class EnemyTankBulletRenderer {
    /**
     * 绘制坦克子弹外观（美化版：多层发光效果）
    ...
    static render(graphics: Graphics, size: number): void {
        ...
        // 外层发光
        graphics.fillColor = new Color(255, 0, 0, 128); // rgba(255, 0, 0, 0.5)
        graphics.circle(0, 0, radius * 1.8);
        graphics.fill();
        
        // 中层发光
        graphics.fillColor = new Color(255, 0, 0, 179); // rgba(255, 0, 0, 0.7)
